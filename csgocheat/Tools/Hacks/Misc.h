...
public:
	...
	static void EdgeJump()
	{
		if (Vars.Ragebot.EdgeJump) {
			auto LocalP = Interfaces.pEntList->GetClientEntity(Interfaces.pEngine->GetLocalPlayer());

			if (LocalP->GetMoveType() == MOVETYPE_LADDER || LocalP->GetMoveType() == MOVETYPE_NOCLIP)
				return;

			Vector Start, End;
			Start = Hacks.LocalPlayer->GetVecOrigin();

			VectorCopy(Hacks.LocalPlayer->GetVecOrigin(), Start);
			...
			Interfaces.pTrace->TraceRay(ray, MASK_PLAYERSOLID_BRUSHONLY, &filter, &trace);

			int EdgeJumpKey = VK_MENU;
	...
	//Interfaces.pEntList->GetClientEntityFromHandle((HANDLE)weapon->GetOwnerHandle());
	...
	static void AnimFix()
	{
		// lol pasted in from csgocheat v3 because it looked like it had more
		for (int i = 1; i < 65; ++i) {
			auto entity = Interfaces.pEntList->GetClientEntity(i);
			/*if (!entity->IsValid(true))
				continue;*/
			if (!entity || /*entity != Hacks.LocalPlayer||*/ !entity->isAlive()
				|| entity->IsDormant())
				continue;
