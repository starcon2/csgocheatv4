...
void __stdcall Hooked_PaintTraverse(unsigned int vguiPanel, bool forceRepaint, bool allowForce)
{
	...
	if (Interfaces.pEngine->IsConnected() && Interfaces.pEngine->IsInGame()) 
	{
		...
		*s_bOverridePostProcessingDisable = Vars.Visuals.Effects.NoPostProcessing;
		...
		ConVar* Meme = Interfaces.g_ICVars->FindVar("mat_drawgray");
		SpoofedConvar* meme_spoofed = new SpoofedConvar(Meme);
		if (Vars.Ragebot.Grayscale)
			...
			case FRAME_NET_UPDATE_START:
				...
				if (Vars.Visuals.Effects.NoSmoke) {
					...
					for (auto material_name : smoke_materials) {
						IMaterial* mat = Interfaces.pMaterialSystem->FindMaterial(material_name, "Other textures");
						mat->SetMaterialVarFlag(MATERIAL_VAR_NO_DRAW, Vars.Visuals.Effects.NoSmoke ? true : false);
					}
